...
pub fn apply_maritime_friction(
    mut query: Query<
        (&mut ShipMovement, &mut ShipTrading, &ShipProfile),
        With<ShipMarker>,
    >,
    ...
    let crowding_factor = (num_ships / target_population).max(0.35);
    let global_friction_mult = planning_tuning.0.global_friction_mult * crowding_factor;

    query.par_iter_mut().for_each(|(mut movement, mut trading, profile)| {
        // Labor cost
        let (_, _, labor_mult, _) = ShipState::profile_multipliers_static(profile.archetype);
        let efficiency_factor_labor = (1.20 - 0.35 * profile.efficiency_rating).clamp(0.70, 1.15);
        let labor_rate = crate::ship::BASE_LABOR_RATE_PUB * labor_mult * efficiency_factor_labor;
        ...
        let efficiency_factor_wear = (1.20 - 0.40 * profile.efficiency_rating).clamp(0.65, 1.15);
        let wear_rate = crate::ship::BASE_WEAR_RATE_PUB * wear_mult * efficiency_factor_wear;
        ...
        let wear = movement.last_step_distance.max(0.0) * wear_rate * global_friction_mult * wear_multiplier;
